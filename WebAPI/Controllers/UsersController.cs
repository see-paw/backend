...
namespace WebAPI.Controllers
{
    ...
    public class UsersController(IUserAccessor userAccessor, IMapper mapper) : BaseApiController
    {
        ...
        /// This includes user ID, email, name, role, and shelter ID (if AdminCAA).
        /// </summary>
        /// <returns>
        /// A <see cref="ResCurrentUserDto"/> object containing all authenticated user's data.
        /// </returns>
        [Authorize(Roles = "AdminCAA, User")]
        [HttpGet("me")]
        public async Task<ActionResult<ResCurrentUserDto>> GetCurrentUser()
        {
            ...
            var userDto = mapper.Map<ResCurrentUserDto>(result.Value);

            return HandleResult(Result<ResCurrentUserDto>.Success(userDto, 200));
        }
